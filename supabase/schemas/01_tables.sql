...
-- Fleet: machines owned by clients
...
-- Active maintenance contracts on machines
create table public.machine_contracts (
    id bigint generated by default as identity primary key,
...
-- Add Tondix columns to deals table
alter table public.deals add column if not exists deal_type text not null default 'tondeuse';
alter table public.deals add column if not exists product_id bigint references public.products(id) on delete set null;
alter table public.deals add column if not exists service_id bigint references public.services(id) on delete set null;
alter table public.deals add column if not exists machine_id bigint references public.machines(id) on delete set null;
